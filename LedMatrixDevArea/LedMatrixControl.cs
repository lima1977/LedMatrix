  ...
  class LedMatrixItem
  {
    ...
    public void MoveItem(uint p_uiTickCount,int p_iNbCtrlLedLine,int p_iNbCtrLedRow)
    {
      ...
      switch (m_sSpeed)
      {
        ...
        case ItemSpeed.Normal:
          {
            iOffset = 1;
            break;
          }
        case ItemSpeed.Rapido:
          {
            iOffset = 2;
            break;
          }
      ...
      switch (m_dDirection)
      {
        case ItemDirection.Cima:
          m_pCurrentLocation.Y -= iOffset;
          break;
        case ItemDirection.Baixo:
          m_pCurrentLocation.Y += iOffset;
          break;
        case ItemDirection.Esquerda:
          m_pCurrentLocation.X -= iOffset;
          break;
        case ItemDirection.Direita:
          m_pCurrentLocation.X += iOffset;
          break;
        default:
          break;
    ...
    protected override void OnPaint(PaintEventArgs pe)
    {
      ...
      for (int iIdxLine = 0; iIdxLine < m_llLedPropertyList.Count; iIdxLine++)
      {
        ...
        for (int iIdxRow = 0; iIdxRow < m_llLedPropertyList[0].Count; iIdxRow++)
        {
          ...
          if (bLedOn == true)
          {
            if (m_lsLedStyle == LedSyle.Quadrado)
            {
